...
async function handleCheckUpdate(context, eventName, event) {
  ...
  if (action === 'completed') {
    logger.info("A status check is not yet complete:", eventName, state);
    throw new NeutralExitError();
  } else {
    if (payload.conclusion === 'success') {
      await updateAndMergePullRequest(context, payload.pull_requests[0]);
    } else {
      logger.info("A status check completed unsuccessfully:", eventName, state);
      throw new NeutralExitError();
    }
  }
}
